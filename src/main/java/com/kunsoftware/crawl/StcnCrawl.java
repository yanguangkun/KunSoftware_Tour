...
public class StcnCrawl {

	private static Logger logger = LoggerFactory.getLogger(StcnCrawl.class);	
	private static String parentLink = "http://www.stcn.com/";
	public static void main(String[] args) {
		...
		try {
			...
			Document doc = Jsoup.connect(parentLink)
					.userAgent("Mozilla/5.0 (Windows NT 6.3; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/30.0.1599.69 Safari/537.36")
					.get();
			Elements hrefs = doc.select(".cnt_A .cnt21").select("h3,.info,.ui-tab-cnt").select("a[href]");
